...
void cuSZp_compress_plain_f64(double* d_oriData, unsigned char** d_cmpBytes, size_t nbEle, size_t* cmpSize, double errorBound, cudaStream_t stream)
{
    ...
    dim3 gridSize(gsize);
    cuSZp_compress_kernel_plain_f64_profiling<<<gridSize, blockSize, sizeof(unsigned int)*2, stream>>>(d_oriData, d_cmpOffset, d_locOffset, d_flag, errorBound, nbEle);
    ...
    *cmpSize = (size_t)glob_sync + (nbEle+cmp_tblock_size*cmp_chunk-1)/(cmp_tblock_size*cmp_chunk)*(cmp_tblock_size*cmp_chunk)/32;
    ...
    cuSZp_compress_kernel_plain_f64_concatenate<<<gridSize, blockSize, sizeof(unsigned int)*2, stream>>>(d_oriData, *d_cmpBytes, d_cmpOffset, d_locOffset, d_flag, errorBound, nbEle);
