...
void cuSZp_compress_plain_f32(float* d_oriData, unsigned char** d_cmpBytes, size_t nbEle, size_t* cmpSize, float errorBound, cudaStream_t stream)
{
    ...
    dim3 gridSize(gsize);
    cuSZp_compress_kernel_plain_f32_profiling<<<gridSize, blockSize, sizeof(unsigned int)*2, stream>>>(d_oriData, d_cmpOffset, d_locOffset, d_flag, errorBound, nbEle);
    ...
    *cmpSize = (size_t)glob_sync + (nbEle+cmp_tblock_size*cmp_chunk-1)/(cmp_tblock_size*cmp_chunk)*(cmp_tblock_size*cmp_chunk)/32;
    ...
    cudaMalloc((void**)d_cmpBytes, sizeof(unsigned char)*(*cmpSize));

    // Second compression kernel
    cuSZp_compress_kernel_plain_f32_concatenate<<<gridSize, blockSize, sizeof(unsigned int)*2, stream>>>(d_oriData, *d_cmpBytes, d_cmpOffset, d_locOffset, d_flag, errorBound, nbEle);
