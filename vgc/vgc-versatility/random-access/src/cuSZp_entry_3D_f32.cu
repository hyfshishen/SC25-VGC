...
void cuSZp_decompress_3D_plain_f32(float* d_decData, unsigned char* d_cmpBytes, size_t nbEle, size_t cmpSize, uint3 dims, float errorBound, cudaStream_t stream)
{
    ...
    // Used for random access evaluation.
    int tarWarp = gsize/2;
    ...
    dim3 gridSize(gsize);
    if(dims.x % 4 == 0)
        cuSZp_decompress_kernel_3D_plain_vec4_f32<<<gridSize, blockSize, sizeof(unsigned int)*2, stream>>>(d_decData, d_cmpBytes, d_cmpOffset, d_locOffset, d_flag, blockNum, dims, errorBound, tarWarp);
    else if(dims.x % 2 == 0)
        cuSZp_decompress_kernel_3D_plain_vec4_f32<<<gridSize, blockSize, sizeof(unsigned int)*2, stream>>>(d_decData, d_cmpBytes, d_cmpOffset, d_locOffset, d_flag, blockNum, dims, errorBound, tarWarp);
    else
        cuSZp_decompress_kernel_3D_plain_vec4_f32<<<gridSize, blockSize, sizeof(unsigned int)*2, stream>>>(d_decData, d_cmpBytes, d_cmpOffset, d_locOffset, d_flag, blockNum, dims, errorBound, tarWarp);
